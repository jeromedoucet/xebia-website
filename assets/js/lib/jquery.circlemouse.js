        ...
        _initEvents: function () {
            ...
            this.$el.on({
                'mouseenter.circlemouse': function (event) {
                    ...
                    // save cursor type
                    var cursor = 'default';
                    var context = _self.options.context || _self;

                    if (_self.$el.css('cursor') === 'pointer' || _self.$el.is('a')) cursor = 'pointer';
                ...
                'mouseleave.circlemouse': function (event) {
                    var el = $(this);
                    var context = _self.options.context;
                ...
                'click.circlemouse': function (event) {
                    ...
                    if (!el.data('inside')) return false;
                    else _self.options.onClick(event, _self.$el, context);
                }
            });
    ...
    $.fn.circlemouse = function (options) {
        if (typeof options === 'string') {
            ...
            this.each(function () {
                ...
                if (!instance) {
                    logError("cannot call methods on circlemouse prior to initialization; " + "attempted to call method '" + options + "'");
                    return;
                }
